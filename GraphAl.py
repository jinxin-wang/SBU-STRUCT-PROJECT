...
class weight:
    def __init__(self, vertex1, vertex2):
        ...
        self.S1S2 = np.linalg.norm(vertex1[0].coord - vertex2[0].coord)
        self.S1E2 = np.linalg.norm(vertex1[0].coord - vertex2[1].coord)
        self.S2E1 = np.linalg.norm(vertex1[1].coord - vertex2[0].coord)
        self.q = self.get_dihedral(vertex1[1].coord, vertex1[0].coord,
                                   vertex2[0].coord, vertex2[1].coord)
...
#edge being a list of 3 elements: [vertex1, vertex2, weight]
class edge:
    def __init__(self, vertex1, vertex2):
        self.v1 = vertex1
        self.v2 = vertex2
...
def debug_vertice_type(edges_dict):
...
def select_aa_acessible(pdb_fname,chain_name=None):
    ...
    if chain_name :
        return (dssp_dict,[ key for key in key_list if chain_name in key and dssp_dict[key][2] >= 1 ])
    else :
        return (dssp_dict,[ key for key in key_list if dssp_dict[key][2] >= 1 ])
    

def select_vertices(structure):
    vertices_list = []
    for residu in structure[0].get_residues() :
        if Bio.PDB.is_aa(residu.get_resname()):
    ...
    while len(nodes) :
        ...
        for node in sg:
            if node not in nodes:
                raise ValueError("Node going to be removed is NOT in the list !")
            nodes.remove(node)
    return subgraphs
...
def build_graph_sub(pdb_fname, chain = None): 
    ...
    (d,ks) = select_aa_acessible(pdb_fname, chain)
    vertices_list = select_vertices(structure)
    graph = build_graph(vertices_list)
    return graph
